---
title: "qps: Introduction and Quickstart"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{qps: Introduction and Quickstart}
...
# Package Quickstart
Applying the package within your workflow to estimate treatment effects is fairly straightforward, and requires just two function calls -- `estimate_qps()` and `estimate_treatment_effect`. 
```{r, eval=F}
...
summary(ivreg_model)
```
`estimate_qps()` has support for parallelized estimation, mixed variables with some discrete values, input variable inference, and seeding.
```{r, eval=F}
...
```
The qps package also offers a covariate balance test diagnostic.
```{r, eval=F}
